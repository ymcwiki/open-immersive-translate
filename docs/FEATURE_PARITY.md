...
说明：单元测试、组件测试或端到端测试均可支撑 ✅；仅有适配器匹配测试、尚未在真实第三方站点验证的兼容项仍记为 🔧。
...
| A29 | 按语言指定翻译模式                                     | ✅   | 3    |
| A30 | 分标签页状态角标（idle/translating/done/error + 计数） | ✅   | 3    |
...
| C5  | Popup：更多菜单（设置、快捷键、缓存、反馈、侧边栏、PDF、字幕、配置导入导出）                                                                                 | ✅   | 3    |
| C6  | 悬浮球（拖动、右键菜单、位置记忆）                                                                                                                           | ✅   | 1    |
| C7  | 右键菜单（网页、选中文本、PDF、字幕文件、侧边栏）                                                                                                            | ✅   | 2    |
| C8  | 鼠标悬停 + 修饰键翻译单段                                                                                                                                    | ✅   | 1    |
...
E4–E6 的离线验证边界：每个列出的站点适配器都有基于实际捕获格式的解析单测，覆盖 TTML、WebVTT 和 Bilibili JSON。以下内容仍未验证，因此继续标记为 🔧：

- E4：带登录态和 DRM 的真实播放流程、各平台当前字幕接口，以及播放器更新后的持续兼容性。
- E5：需要登录的课程播放器、课程内字幕接口，以及真实播放中的字幕切换和时间轴同步。
- E6：Twitter Spaces 实时字幕、Facebook/Dailymotion 播放器变体，以及 Bilibili 当前线上接口的端到端拦截。
...
| G13 | 搜索增强（Google / Bing / DuckDuckGo 英语 query）       | ✅   | 3    |
| G14 | Firefox 打包                                            | ✅   | 3    |
| G15 | 油猴脚本版                                              | ✅   | 3    |
